...
T69|~|PROC COEFFICIENT — `WeaponDefinition.procCoef` (default 1); route on-hit status/trigger via `chance·duration·magnitude × procCoef`; family coefs (rotary~0.5 … ballistic 1.0 … orbital~3.0). Proc chains depth-guarded (`MAX_PROC_DEPTH`, inherited reduced coef `PROC_CHAIN_INHERIT`), single-Rng deterministic. Thread `depth`+`procCoef` through `effects.ts` TriggerCtx. ties T42|V32,V21,V3,§I.data
T70|.|DOT-AS-%-OF-HIT — damaging-ailment dps = `COEF × hitDamage / duration` (burn/bleed scale w/ weapon, replaces flat `3 dps`); per-status stacking rule {independent|strongest|count-capped}; modifier ailments capped (shock→+dmg-taken ≤0.5, brittle→+crit ≤0.06, sap/scorch). mark amp folds in. extends T39/T52|V33,V3
T71|.|DRAFT RESOURCES+ — Lock (carry an offered card to next draft, survives reroll) + tag-banish (drop all cards w/ a tag, ⊥ empty pool V11); each bounded per-run; permanent sources (Retainer Clause→lock, Blacklist Rights tier-2→tag-banish). ? tier-first rarity roll (`50/25/12/5/4/1` then in-tier synergy pick) once pools wide enough. extends T41|V11,§I.data
